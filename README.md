...
Qwen3.8-27B inference on Apple Silicon through llama.cpp Metal.
...
```sh
QWEN_EXTRA_SYSTEM_PROMPT_FILE=/absolute/path/to/instructions.md qwen-local
```
...
Keep confidential instructions outside this public repository. They remain visible to Qwen Code session history and local process inspection while the launcher is running. Do not use a prompt file as a security boundary; tool approvals, sandboxing, and network controls remain authoritative.
